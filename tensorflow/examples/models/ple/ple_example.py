...
if __name__ == "__main__":
    data_path = os.path.dirname(root_path) + "/data/census-income.sample"
    #print(data_path)
    ...
    dnn_feature_colums = fixlen_feature_columns
    linear_feature_colmns = fixlen_feature_columns

    feature_names = get_feature_names(linear_feature_colmns + dnn_feature_colums)

    # generate input data for model
    train, test = train_test_split(data, test_size=0.2, random_state=2012)
    train_model_input = {name:train[name] for name in feature_names}
    test_model_input = {name:test[name] for name in feature_names}

    # define Model, train, predict and evaluate
    model = PLE(dnn_feature_colums, shared_expert_num=2, specific_expert_num=2, num_levels=2,
