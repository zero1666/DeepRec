...
def create_embedding_dict(sparse_feature_columns, varlen_sparse_feature_columns, l2_reg,
    ...
    for feat in sparse_feature_columns:
        emb = Embedding(feat.vocabulary_size, feat.embedding_dim,
                       embedding_initializer=feat.embedding_initializer,
                       embedding_regularizer=l2(l2_reg),
...
def create_embedding_matrix(feature_columns, l2_reg, prefix='', seq_mask_zero=True):
    from . import feature_column as feat_lib
    sparse_feature_columns = list(
        filter( lambda x: isinstance(x, feat_lib.SparseFeat), feature_columns)) if feature_columns else []
    varlen_feature_columns = list(
        filter( lambda x: isinstance(x, feat_lib.VarLenSparseFeat), feature_columns)) if feature_columns else []
    sparse_embedding_dict = create_embedding_dict(sparse_feature_columns, varlen_sparse_feature_columns,
...
def embedding_lookup(sparse_embedding_dict, sparse_input_dict, sparse_feature_columns, return_feat_list=(),
    ...
    for feat in sparse_feature_columns:
        ...
        if len(return_feat_list) == 0 or feat_name in return_feat_list:
            if fg.use_hash:
                lookup_idx = Hash(feat.vocabulary_size, mask_zero=(feat_name in mask_feat_list),
                                  vocabulary_path=feat.vocabulary_path)(input_dict[feat_name])
            else:
                lookup_idx = input_dict[feat_name]
