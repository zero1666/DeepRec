...
from tensorflow.python.keras.models import Mdoel
from tensorflow.python.keras.layers import Dense, Lambda

from ..layers.core import PredictionLayer, DNN
from ..layers.utils import combined_dnn_input, reduce_sum
from ..utils.feature_column import build_input_features, input_from_feature_columns

def PLE(dnn_feature_columns, shared_expert_num=1, specific_expert_num=1, num_level=2,
       expert_dnn_hidden_units=(256,), tower_dnn_hidden_units=(64,), gate_dnn_hidden_units=(),
       l2_reg_embedding=0.0001, l2_reg_dnn=0, seed=1024, dnn_dropout=0, dnn_activation='relu',
       dnn_use_bn = False, task_types=('binary', 'binary'), task_names = ('ctr', 'cvr')):
    ...
    def cgc_net(inputs, level_name, is_last=False):
        ...
        for i in range(num_tasks):
            for j in range(specific_expert_num):
                expert_network = DNN(expert_dnn_hidden_units, dnn_activation, 
                                     l2_reg_dnn, dnn_dropout, dnn_use_bn, seed=seed,
                                     name=level_name + 'task_' + task_names[i]+ '_expert_specific' + str(j))(inputs[i])
                specific_expert_outputs.append(expert_network)
        
        # build share_expert layer
        shared_expert_outputs = []
        for k in range(shared_expert_num):
            expert_network = DNN(expert_dnn_hidden_units, dnn_activation, 
                                     l2_reg_dnn, dnn_dropout, dnn_use_bn, seed=seed,
                                     name=level_name + '_expert_shared' + str(k))(inputs[-1])
        ...
        for i in range(num_tasks):
            ...
            cur_expert = specific_expert_outputs[i*specific_expert_num:(i+1)*specific_expert_num] + shared_expert_outputs
            #
            expert_concat = Lambda(lambda x:tf.stack(x, axis=1))(cur_expert)
            # build gate layers
            gate_input = DNN(gate_dnn_hidden_units, dnn_activation, l2_reg_dnn,dnn_dropout,dnn_use_bn,
                            seed=seed, name = level_name+ 'gate_specific_' + task_names[i])(inputs[i])# gate[i] for task input[i]
        ...
        if not is_last:
            ...
            cur_expert = specific_expert_outputs + shared_expert_outputs
            
            expert_concat = Lambda(lambda x: tf.stack(x, axis=1))(cur_expert)
            
            # build gate layers
            gate_input = DNN(gate_dnn_hidden_units, dnn_activation,l2_reg_dnn, dnn_dropout, dnn_use_bn,
                            seed=seed,  name=level_name + 'gate_shared')(inputs[-1])
            gate_out = Dense(cur_expert_num, use_bias=False, activation='softmax',
            ...
            # gate multiply the expert
            gate_mul_expert = Lambda(lambda x: reduce_sum(x[0]*x[1], axis=1, keep_dims=False),
                                    name=name=level_name + 'gate_softmax_shared')([expert_concat, gate_out])
    ...
    ple_input = [dnn_input] *(num_tasks+1) # [task1, task2, ... taskn, shared task]
    ple_outputs = []
    for i in range(num_level):
        if i == num_level -1 : # the last level
    ...
    model = Model(inputs=input_list, output=task_outs)
    return model