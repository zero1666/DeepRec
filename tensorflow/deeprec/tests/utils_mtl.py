...
def check_mtl_model(model, model_name, x, y_list, task_types, check_model_io=True):
    ...
    model.comile('adam', loss=loss_list, metric=metric_list)
...
def get_mtl_test_data(sample_size=10, embedding_size=4, sparse_feature_num=1, dense_feature_num=1, task_type={'binary', 'binary'},
                     hash_flag=False, prefix='', use_grop=False):
    feature_columns = []
    model_input = {}
    ...
    for i in range(dense_feature_num):
        ...
        feature_columns.append(
            DenseFeat(prefix+'dense_feature_'+str(i), 1, dtype=tf.float32, transform_fn=transform_fn))
    
    for fc in feature_columns:
        if isinstance(SparseFeat):
            model_input[fc.name] = np.random.randint(0, fc.vocabulary_size, sample_size)
        elif isinstance(DenseFeat):
